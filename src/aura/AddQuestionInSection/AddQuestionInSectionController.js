({
    ...
    saveQuestionInSection:function(component, event, helper){
        ...
        var prevCol = parseInt(qstnQnaireObject.GKN_FB__Related_To_Column__c.replace('col',''),10);
        var sectionList = component.get("v.lstQuesGroup");
        
        for(var i=0;i<sectionList.length;i++){
            //console.log(sectionList[i].GKN_FB__Question_Group__c+'=='+selectedSectionId)
            if(sectionList[i].GKN_FB__Question_Group__c===selectedSectionId){
                totalCol = sectionList[i].GKN_FB__Question_Group__r.GKN_FB__No_Of_Columns__c;
