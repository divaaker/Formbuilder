        ...
        if (selectedOptionBranching !== undefined && selectedOptionBranching.length > 0) {
            for (var i = 0; i < selectedOptionBranching.length; i++) {
                var objQuestinnaireDynLogic = {
                    ...
                    GKN_FB__Question_Option__c : selectedOptionBranching[i].qstnOptionId,
                    GKN_FB__Question_Group__c : vSectionId,
                    GKN_FB__Question_Questionnaire__c : selectedOptionBranching[i].question_QuestionnaireID,
                    GKN_FB__Show_Question_Questionnaire__c : selectedOptionBranching[i].show_Question_QuestionnaireID
                };
            ...
            if (state === "SUCCESS") {
                ...
                if (lstQnnaireDynLogic !== undefined && lstQnnaireDynLogic.length > 0) {
                    for (var i = 0; i < lstQnnaireDynLogic.length; i++) {
                        var branchingMap = {
                            qstnOptionId: lstQnnaireDynLogic[i].GKN_FB__Question_Option__c,
                            question_QuestionnaireID: lstQnnaireDynLogic[i].GKN_FB__Question_Questionnaire__c,
                            show_Question_QuestionnaireID: lstQnnaireDynLogic[i].GKN_FB__Show_Question_Questionnaire__c
                        };
