        ...
        $( "#sortable" ).sortable({
            ...
            update: function(e, ui){
				...
				if(newIndex>oldIndex){
                    lstQuestnGrp[oldIndex].GKN_FB__Sort_Order__c=newIndex;
                    for(var i=upQOrder;i>=oldIndex;i--){
                        if(lstQuestnGrp[i].Id!==lstQuestnGrp[oldIndex].Id){
                            upQOrder--;
							lstQuestnGrp[i].GKN_FB__Sort_Order__c=upQOrder;
                ...
                // When section from top to bottom
					...
					for(var j=upQOrder;j<=oldIndex;j++){
                        if(lstQuestnGrp[j].Id!==lstQuestnGrp[oldIndex].Id){
                            upQOrder++;
							lstQuestnGrp[j].GKN_FB__Sort_Order__c=upQOrder;
                            console.log(lstQuestnGrp[j]);
						}
                    }
                }
                lstQuestnGrp.sort(function(a,b){
                    if (a.GKN_FB__Sort_Order__c < b.GKN_FB__Sort_Order__c)
                    	return -1;
                  	if (a.GKN_FB__Sort_Order__c > b.GKN_FB__Sort_Order__c)
                    	return 1;
                  	return 0;
