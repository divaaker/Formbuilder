({
    doInit: function(component, event, helper) {
        ...
        if (isEditQue === false) {
            ...
            component.set("v.lstSwitchQuestionOptions", qes.GKN_FB__Question_Options__r);
            var QOption = component.get("v.lstSwitchQuestionOptions");
            for (var index = 0; index < QOption.length; index++) {
                if (index === 0) {
                    component.set("v.optionFirstName", QOption[index].GKN_FB__Name__c);
                    component.set("v.optionFirstAlias", QOption[index].GKN_FB__Value__c);
                    component.set("v.optionFirstscore", QOption[index].GKN_FB__Score__c);
                    component.set("v.optionFirstscoreCheck", QOption[index].GKN_FB__Is_Score_Required__c);
                } else if (index === 1) {
                    component.set("v.optionSecondName", QOption[index].GKN_FB__Name__c);
                    component.set("v.optionSecondAlias", QOption[index].GKN_FB__Value__c);
                    component.set("v.optionSecondscore", QOption[index].GKN_FB__Score__c);
                    component.set("v.optionSecondscoreCheck", QOption[index].GKN_FB__Is_Score_Required__c);
                }
            }
    ...
    saveQues: function(component, event, helper) {
        ...
        if(qes.GKN_FB__Weight__c !== undefined && qes.GKN_FB__Weight__c !==0){
            if (!helper.checkNumber(component, event, qes.GKN_FB__Weight__c)) {
